...
dump:
	PGDATABASE=$(PGDATABASE) PGHOST=$(PGHOST) PGPORT=$(PGPORT) PGUSER=$(PGUSER) PGPASSWORD=$(PGPASSWORD) \
	pg_dump --clean --if-exists --create -w --compress=3 -F custom -f fias.dump

restore:
	PGHOST=$(PGHOST) PGPORT=$(PGPORT) PGUSER=$(PGUSER) PGPASSWORD=$(PGPASSWORD) \
	pg_restore --dbname=$(PGDATABASE) --format=custom -x --clean --if-exists --exit-on-error -v fias.dump